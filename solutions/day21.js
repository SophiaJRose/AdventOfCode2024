export function partOne(input, numDirectional) {
    return solution(input, 1);
}
export function partTwo(input, numDirectional) {
    return solution(input, 24);
}
const fastestRoute = new Map([
    ["AA", ["A"]],
    ["A^", ["<A"]],
    ["A>", ["vA"]],
    ["Av", ["<vA", "v<A"]],
...
function solution(input, numDirectional) {
    let codes = [];
    try {
    ...
    try {
        ...
        for (let code of codes) {
            let numericMoves = getNumericMovements(code);
            let transitionCosts = new Map();
            ...
            for (let key of numericMoves) {
                ...
                let curCost = transitionCosts.get(curTransition);
            ...
            let numericValue = Number(code.slice(0, code.length - 1));
...
function getNumericMovements(keyPresses) {
    ...
    keypad.set("0", [3, 1]);
    keypad.set("A", [3, 2]);
    let pos = keypad.get("A");
    let movements = "";
    for (let key of keyPresses) {
        let desiredPos = keypad.get(key);
        if (desiredPos == undefined) {
            throw new Error(`Desired key does not exist on numeric keypad: ${key}`);
        }
        let vertMove = pos[0] - desiredPos[0];
        let horizMove = pos[1] - desiredPos[1];
...
function updateTransitionCosts(transitionCosts) {
    let newCosts = new Map();
