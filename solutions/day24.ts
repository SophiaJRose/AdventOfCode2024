...
export function partOne(input: string): string {
    let wires: Map<string, boolean> = new Map();
    let gates: Gate[] = [];
    try {
        let sections: string[] = input.trim().split("\n\n");
        let startingWires: string[] = sections[0].split("\n");
        for (let wire of startingWires) {
            let wireSections: string[] = wire.split(": ")
            let wireName: string = wireSections[0];
            let wireValue: number = Number(wireSections[1]);
        ...
        let gateStrs: string[] = sections[1].split("\n");
        for (let gateStr of gateStrs) {
            let gateSections: string[] = gateStr.split(" -> ");
            let gateInputs: string[] = gateSections[0].split(" ");
            let gateType: string = gateInputs[1];
            if (gateType != "AND" && gateType != "OR" && gateType != "XOR") {
                throw new Error(`Invalid gate type ${gateType} in gate ${gateStr}`);
            }
            let gate: Gate = {gateType: gateInputs[1], input1: gateInputs[0], input2: gateInputs[2], output: gateSections[1]};
            gates.push(gate);
        }
    } catch (err) {
        console.log(err);
    ...
    try {
        let queue: Gate[] = gates.slice();
        while (queue.length != 0) {
            let curGate = <Gate>queue.shift();
            let input1: boolean | undefined = wires.get(curGate.input1);
            let input2: boolean | undefined = wires.get(curGate.input2);
            // If either of the input gates of the queue is not known, put back at end of queue and check next gate
            if (input1 === undefined || input2 === undefined) {
                queue.push(curGate);
                continue;
            }
            let gateType: string = curGate.gateType;
            let output: boolean;
            switch(gateType) {
                case "AND":
            ...
            let outputWire: string = curGate.output;
        ...
        console.log(zValues);
        let zNumber: number = parseInt(zValues.reverse().join(""), 2);
...
export function partTwo(input: string): string {
    ...
    try {
        ...
        let rule1: Gate[] = gates.filter((x) => x.output.startsWith("z") && x.output != "z45" && x.gateType != "XOR");
        let rule1Outputs = rule1.map((x) => x.output);
        console.log(rule1);
        let rule2: Gate[] = gates.filter((x) => !x.output.startsWith("z") && !x.input1.startsWith("x") && !x.input1.startsWith("y") && !x.input2.startsWith("x") && !x.input2.startsWith("y") && x.gateType == "XOR");
        let rule2Outputs = rule2.map((x) => x.output);
        console.log(rule2);
        let rule3: Gate[] = gates.filter((x) => {
        ...
        let swappedOutputs: string[] = rule1Outputs.concat(rule2Outputs).concat(rule3Outputs);
