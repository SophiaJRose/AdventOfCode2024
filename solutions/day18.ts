...
export function partOne(input: string): string {
    ...
    try {
        ...
        for (let i = 0; i < numBytes; i++) {
            ...
            let byteNode: Node | undefined = graph.nodes.find((x) => x.position[0] == byte[1] && x.position[1] == byte[0]);
        ...
        dijkstra(graph);
        let endNode: Node | undefined = graph.nodes.find((x) => x.position[0] == height-1 && x.position[1] == width-1);
...
function buildGraph(height: number, width: number): Graph {
    let graph: Graph = {nodes: []};
    let startNode: Node = {position: [0,0], up: undefined, down: undefined, left: undefined, right: undefined, distance: 0, distPath: [[0,0]]};
    graph.nodes.push(startNode);
    let queue: Node[] = [startNode];
    while (queue.length != 0) {
        let curNode: Node = <Node>queue.shift();
        if (curNode.up === undefined) {
            if (curNode.position[0] == 0) {
                ...
                if (existingNode != undefined) {
                    ...
                    let upNode: Node = {position: upPos, up: undefined, down: curNode, left: undefined, right: undefined, distance: Number.POSITIVE_INFINITY, distPath: []}
                    curNode.up = upNode;
        ...
        if (curNode.down === undefined) {
            if (curNode.position[0] == height-1) {
                ...
                if (existingNode != undefined) {
                    ...
                    let downNode: Node = {position: downPos, up: curNode, down: undefined, left: undefined, right: undefined, distance: Number.POSITIVE_INFINITY, distPath: []}
        ...
        if (curNode.left === undefined) {
            if (curNode.position[1] == 0) {
                ...
                if (existingNode != undefined) {
                    ...
                    let leftNode: Node = {position: leftPos, up: undefined, down: undefined, left: undefined, right: curNode, distance: Number.POSITIVE_INFINITY, distPath: []}
        ...
        if (curNode.right === undefined) {
            if (curNode.position[1] == width-1) {
                ...
                if (existingNode != undefined) {
                    ...
                    let rightNode: Node = {position: rightPos, up: undefined, down: undefined, left: curNode, right: undefined, distance: Number.POSITIVE_INFINITY, distPath: []}
