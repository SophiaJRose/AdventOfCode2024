export function partOne(input: string): string {
    ...
    try {
        let output: string[] = runProgram(registerA, registerB, registerC, program);
        return output.join(",");
    } catch (err) {
        console.log(err);
...
function runProgram(registerA: number, registerB: number, registerC: number, program: number[]): string[] {
